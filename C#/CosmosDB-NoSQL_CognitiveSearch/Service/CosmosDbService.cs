...
public class CosmosDbService
{
    ...
    {
        QueryDefinition query = new QueryDefinition("SELECT value Count(c.id) FROM c WHERE IS_ARRAY(c.embedding)=@status")
    ...
    public async Task UpdateRecipesAsync(Dictionary<string, float[]> dictInput)
    {
        ...
        {
            await _container.PatchItemAsync<Recipe>(entry.Key,new PartitionKey(entry.Key), patchOperations: new[] { PatchOperation.Add("/embedding", entry.Value)});
        }
    }
